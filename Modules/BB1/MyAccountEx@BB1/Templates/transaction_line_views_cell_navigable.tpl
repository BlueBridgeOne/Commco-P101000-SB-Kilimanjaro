...
<tr class="transaction-line-views-cell-navigable {{cellClassName}} item-{{itemId}}" data-id="{{itemId}}" data-item-type="{{itemType}}">
	...
	<td class="transaction-line-views-cell-navigable-details" name="item-details">
		<p class="transaction-line-views-cell-navigable-product-name">
			{{#if isNavigable}}
				<a class="transaction-line-views-cell-navigable-product-title-anchor" {{{itemURLAttributes}}}>{{item.cseg_bb1_manufactur.text}} {{itemName}}</a>
			{{else}}
				<span class="transaction-line-views-cell-navigable-product-title">
					{{item.cseg_bb1_manufactur.text}} {{itemName}}
				</span>
			{{/if}}
		</p>
		<p>{{item.salesdescription.value}}</p>
		{{!--<p>
			<div data-view="Item.Price"></div>
		</p>--}}
		<div data-view="Item.Sku"></div>
		{{#if item.custitem_bb1_msrp.value}}
		<p>  <span class="transaction-line-views-cell-navigable-item-unit-price-label">MSRP price: </span>  <span class="transaction-line-views-cell-navigable-item-reason-value">{{formatCurrency item.custitem_bb1_msrp.value}}</span></p>
		{{/if}}
