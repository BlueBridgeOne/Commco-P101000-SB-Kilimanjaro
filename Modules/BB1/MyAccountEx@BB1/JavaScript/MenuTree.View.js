...
define('MenuTree.View', [
	'Backbone.CompositeView', 'Backbone.CollectionView', 'MenuTree.Node.View', 'Singleton'

	, 'menu_tree.tpl', 'Backbone', 'underscore', 'jQuery'
], function (

	BackboneCompositeView, BackboneCollectionView, MenuTreeNodeView, Singleton
	...
	return Backbone.View.extend({
		...
		menuClick: function (e, synthetic) {
				var target = jQuery(e.currentTarget),this_expandable = target.closest('[data-type="menu-tree-node-expandable"]'), all_expandables = this.$('[data-type="menu-tree-node-expandable"]'), isOpen = this_expandable.hasClass("open");

				all_expandables.removeClass('open');
				all_expandables.removeClass('active');
				if(synthetic&&synthetic.synthetic){
					this_expandable.addClass('active');
					return;
				}
				if (!isOpen) {
