...
define(
	'Transaction.Model', [
		'SC.Model', 'SC.Models.Init', 'Application', 'Utils', 'underscore', 'CustomFields.Utils'
	],
	function (
		SCModel, ModelsInit, Application, Utils, _, CustomFieldsUtils
	) {
		'use strict';
		...
		return SCModel.extend({
			...
			list: function (data) {
					...
					for (var i = 0; i < customColumns.length; i++) {
						this.columns[customColumns[i]] = new nlobjSearchColumn(customColumns[i]);
					}
					...
					if (this.data.from && this.data.to) {
						...
						this.filters.date = [
							'trandate', 'within', new Date(
								this.data.from[0], this.data.from[1] - 1, this.data.from[2]
							), new Date(
								this.data.to[0], this.data.to[1] - 1, this.data.to[2]
							)
						];
					...
					var records = _.map((this.data.page === 'all' ? this.search_results : this.search_results.records) || [], function (record) {
						...
						for (var i = 0; i < customColumns.length; i++) {
							try {
								result[customColumns[i]] = {
									value: record.getValue(customColumns[i]),
									text: record.getText(customColumns[i])
								};
							} catch (err) {
								nlapiLogExecution("debug", "Unable to add custom column " + customColumns[i], err);
							}
						}
						...
						_.each(options_string.split(split_char_4), function (option_line) {
							...
							options_object.push(
								self.transactionModelGetLineOptionBuilder(
									option_line[0], option_line[2], self.transactionModelGetLineOptionValueBuilder(option_line[4], option_line[3]), option_line[1] === 'T'
								)
							);
						});
					}
			...
			loadItemsWithSuiteScript: function (items_to_query) {
					...
					if (items_to_query.length > 0) {
						...
						var filters = [
								new nlobjSearchFilter('entity', null, 'is', nlapiGetUser()), new nlobjSearchFilter('internalid', null, 'is', this.result.internalid), new nlobjSearchFilter('internalid', 'item', 'anyof', items_to_query)
							]
						...
						var customColumns = ["cseg_bb1_manufactur", "salesdescription","custitem_bb1_msrp"];
						for (var i = 0; i < customColumns.length; i++) {
							columns.push(new nlobjSearchColumn(customColumns[i], 'item'));
						}
						...
						_.each(inactive_items_search, function (item) {
							...
							for (var i = 0; i < customColumns.length; i++) {
								loaded_item[customColumns[i]] = {
									value: item.getValue(customColumns[i], 'item'),
									text: item.getText(customColumns[i], 'item')
								};
							}
