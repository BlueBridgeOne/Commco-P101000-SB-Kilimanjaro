...
define(
	'ReorderItems.Model', [
		'SC.Model', 'SC.Models.Init', 'Application', 'StoreItem.Model', 'SiteSettings.Model', 'Utils', 'underscore', 'Transaction.Model'
	],
	function (
		SCModel, ModelsInit, Application, StoreItem, SiteSettings, Utils, _, Transaction
	) {
		'use strict';
		...
		return SCModel.extend({
			...
			search: function (order_id, query_filters) {
				//nlapiLogExecution('DEBUG', 'ITEM SEARCH' ,order_id+" "+JSON.stringify(query_filters));
				var filters = {
					...
					columns = [
						new nlobjSearchColumn('internalid', 'item', 'group'), new nlobjSearchColumn('type', 'item', 'group'), new nlobjSearchColumn('parent', 'item', 'group'), new nlobjSearchColumn('options', null, 'group')
				...
				if (site_settings.isSCISIntegrationEnabled) {
					...
					filters.scisrecords = [
						[
							['type', 'anyof', ['CashSale', 'CustInvc']], 'and', ['createdfrom', 'is', '@NONE@'], 'and', ['location.locationtype', 'is', SC.Configuration.locationTypeMapping.store.internalid], 'and', ['source', 'is', '@NONE@']
						], 'or', [
							['type', 'anyof', ['SalesOrd']]
						]
					];
				...
				if (query_filters.date.from && query_filters.date.to) {
					...
					filters.date = [
						'trandate', 'within', new Date(
							query_filters.date.from[0], query_filters.date.from[1] - 1, query_filters.date.from[2]
						), new Date(
							query_filters.date.to[0], query_filters.date.to[1] - 1, query_filters.date.to[2]
						)
					];
				}
				...
				// nlapiLogExecution('DEBUG', 'ITEM SEARCH' ,"filters="+JSON.stringify(_.values(filters)));
				// nlapiLogExecution('DEBUG', 'ITEM SEARCH' ,"columns="+JSON.stringify(_.values(columns)));
				...
				if (items_info.length) {
					...
					result.records = _.map(result.records, function (line) {
						// prepareStoreItem the collection for the frontend
						//@class ReorderItems.Model.Attributes
						if (StoreItem.get(line.getValue('internalid', 'item', 'group'), line.getValue('type', 'item', 'group'))) {
							return {
